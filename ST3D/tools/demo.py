...
class DemoDataset(DatasetTemplate):
    ...
    def __getitem__(self, index):
        if self.ext == '.bin':
            ...
            #points[:,0,1,2,3]=points[:,[1,0,2,3]]
            #points[:,1]=-points[:,1]
            #print(points)
        elif self.ext == '.pcd':
            #print(str(self.sample_file_list[index]))
            new2 = pypcd.PointCloud.from_path(str(self.sample_file_list[index]))
            new2=new2.pc_data.copy()
            points=np.array([list(new2) for new2 in new2])
            #print("pcd points",points)
        elif self.ext == '.txt':
        ...
        # nromalize intensity
            ...
            tmp_dataset = DataSet('/home/algo-3/work/OpenPCDet/data/pandaset/')
